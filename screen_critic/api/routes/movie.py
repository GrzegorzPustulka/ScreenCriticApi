...
@router.get("/name/{movie_name}", response_model=list[MovieRead], status_code=status.HTTP_200_OK)
...
@router.get("/random", response_model=MovieRead, status_code=status.HTTP_200_OK)
def random_movie(db: Session = Depends(get_db)):
    movie = crud_movie.get_random(db)
    name = crud_category.get(db, movie.category_id).name
    return MovieRead(**jsonable_encoder(movie), category_name=name)
